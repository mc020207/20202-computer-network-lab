...
main()
{
    ...
    while (1) {
        ...
        if (TRACE > 1) {
            ...
            // add by myself, without the nextline more than one sentenec will print at one line
            else printf("\n");
        }
...
float jimsrand()
{
    int mmm = 1 << 15;   /* largest int  - MACHINE DEPENDENT!!!!!!!!   */
    float x;                   /* individual students may need to change mmm */
    x = 1.0*rand() / mmm;            /* x should be uniform in [0,1] */
    return(x);
}
